...
def overlay_graph_on_image(image_array, G, margin=15):
    ...
    for node, (x, y) in adjusted_positions.items():
        cv2.circle(graph_image, (int(x), int(y)), 3, (255, 255, 255), -1)  # Dibujar nodo
    for edge in G.edges():
...
=======
    ...
    result_image = cv2.addWeighted(cropped_image_cv2, 1, graph_image, 1, 0)
    
    return result_image
>>>>>>> 00b98f7225967e9240881011755c77df1dbb40ce
...
plt.imshow(resulting_image_array, cmap='gray')  # Asegúrate de usar cmap='gray' si es una imagen en escala de grises
